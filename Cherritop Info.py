...
def get_config():
    ...
    # Save to pcinfosettings.txt (initially just token and user ID)
...
def restart_reminder():
    ...
    while True:
        ...
        if now - last_reminder >= REMINDER_INTERVAL:
            ...
            last_reminder = now
            # Save updated timestamp to config (add third line if missing)
            with open(CONFIG_FILE, "r") as f:
                lines = f.read().splitlines()
            if len(lines) < 3:
                lines.append(str(last_reminder))
            else:
                lines[2] = str(last_reminder)
            with open(CONFIG_FILE, "w") as f:
                f.write("\n".join(lines))
